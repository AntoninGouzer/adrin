...
/*.sampleHoldDetect:
    ...
    signal : input audio signal, an array of doubles
   N      : number of samples in the input signal
   sampleHoldNum     : threshold number of consecutive samples with the same value
...
{
  int * burst = zeros_ivect(N);
  int sampleHoldCount=0;
  int lastSampleHold = 0;
  int i;
  /*printf("N %d\n",N); */
  ...
  for(i=0; i<N; i++)
  {
    ...
    if(i>0)
   {
	...
	if( signal[i] >= signal[i-1] -  0.00006  && signal[i] <= signal[i-1] +  0.00006 &&  signal[i] >= 0.1  &&  signal[i] <= 1- 0.00006)

	   {
		sampleHoldCount++;

		if(sampleHoldCount>=sampleHoldTresh)
			{

			if(i-lastSampleHold>=mergeTimeHolds)
				{
				burst[i]=1;
				lastSampleHold=i;
				sampleHoldCount=0;
				/*printf("signal[i-1] %0.7f,  signal[i] %0.7f\n",signal[i-1],signal[i]);*/
				/*printf("sampleHoldTresh %d\n",sampleHoldTresh);*/
				}
			else
				{
				sampleHoldCount=0;
...
static void
sample_hold_detect (SNDFILE * infile, FILE * outfile, int channels,int sampleHoldTresh,int mergeTimeHolds)
	...
	{	for (k = 0 ; k < readcount ; k++)
		if (k>0)
		{
			{	for (m = 0 ; m < channels ; m++)
					sprintf (" % 12.10f", buf [k * channels + m]) ;
